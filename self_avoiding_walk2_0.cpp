...
int main(int argc, char **argv)
{
  ...
  int total_steps=100; //cantidad de pasos a llevar a cabo, en un futuro se podría implementar un while y que se desarrolle hasta que el caminante quede encerrado, además de que aún no se ha implementado un "marco" dónde encerrar al caminante
  ...
  std::cout<<average(dimension, total_steps, walks, aux_position, position)<<std::endl;
...
double average(int dimension, int total_steps, int walks, int *aux_position, int *position)
{
  ...
  double prom [walks]={};
  double sum=0;
  for(int ww=0; ww<walks; ww++)
  {
    prom [ww]=final_position(dimension, total_steps, aux_position, position);
    sum+=prom[ww];
