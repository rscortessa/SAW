...
{
  int dimension=3; //dimensiones a utilizar
  int total_steps = std::atoi(argv[1]); //cantidad de pasos a llevar a cabo, en un futuro se podría implementar un while y que se desarrolle hasta que el caminante quede encerrado, además de que aún no se ha implementado un "marco" dónde encerrar al caminante
  ...
  for(int ii=0; ii<total_steps; ++ii)
	...
	{
	  if(aux_position[0]==history[jj][0] && aux_position[1]==history[jj][1] && aux_position[2]==history[jj][2])//este ciclo revisa las entradas del array history y las compara con el valor actual del array aux_position.
	    {
      ...
      else//si proof=0 (No hay posición repetida), se puede dar al array position los valores del nuevo paso (array aux_position) y se procede a imprimirlos y a registrarlos en el historial
	{
      ...
      position[2]=aux_position[2];

	  history[ii+1][0] = position[0];
	  history[ii+1][1] = position[1];
      history[ii+1][2] = position[2];
...
int* random_step(int D, int *r)//esta funcion da un numero aleatorio que puede ser 1 o 0 con igual probabilidad, si es 1 retorna -1 y si es 0 retorna 1.
{

  double step_value = 0;
  int random_number;
  int number;
  random_number = rand() % D;
  number=rand() % D;
  if (number == 2)
    {
      step_value = -1;
